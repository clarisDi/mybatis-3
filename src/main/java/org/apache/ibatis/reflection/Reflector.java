...
public class Reflector {
  ...
  public Reflector(Class<?> clazz) {
    //设置对应的类，type属性,每个 Reflector 对应的类。
    type = clazz;
    // <1> 初始化 defaultConstructor，默认无参构造方法
    addDefaultConstructor(clazz);
    Method[] classMethods = getClassMethods(clazz);
    if (isRecord(type)) {
      //Java 15 和更早版本的 Class.isRecord() 替代方案
  ...
  private void addDefaultConstructor(Class<?> clazz) {
    ...
    //新版mybatis写法
    //找出无参构造方法并赋值给defaultConstructor,前提是存在无参构造方法
  ...
  private void addGetMethods(Method[] methods) {
    ...
    //1.无参的方法,以get和is方法开头 进行遍历
    //PropertyNamer.methodToProperty(m.getName()) eg:getAge -> age
    //2.addMethodConflict对应的解释
  ...
  private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
    // 遍历每个属性，查找其最匹配的方法。因为子类可以覆写父类的方法,包括接口方法，所以一个属性，可能对应多个 getting 方法
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      Method winner = null; //优胜者,即最匹配的方法
      String propName = entry.getKey();
      boolean isAmbiguous = false;
      for (Method candidate : entry.getValue()) {  //candidate --候选人
  ...
  private void addGetMethod(String name, Method method, boolean isAmbiguous) {
    //isAmbiguous为true时，AmbiguousMethodInvoker为MethodInvoker的子类，invoke会抛出ReflectionException异常
  ...
  private void addSetMethods(Method[] methods) {
    ...
    //1.参数数量为1,以set方法开头 进行遍历
    //PropertyNamer.methodToProperty(m.getName()) eg:setAge -> age
    //2.addMethodConflict对应的解释
  ...
  private void addMethodConflict(Map<String, List<Method>> conflictingMethods, String name, Method method) {
    if (isValidPropertyName(name)) {  //判断是合理的属性名
      //1.如果conflictingMethods(map)存在以name为key的值，则直接返回对应的value
      //2.反之，生成一个集合并add(method)放到conflictingMethods(map)中
