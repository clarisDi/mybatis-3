...
public class BeanWrapper extends BaseWrapper {
  ...
  public Object get(PropertyTokenizer prop) {
    //获得集合类型的属性的指定位置的值 例如说：User 对象的 list[0] 。所调用的方法，
    // 都是 BaseWrapper 所提供的公用方法。
  ...
  public boolean hasSetter(String name) {
    ...
    if (prop.hasNext()) {
      if (metaClass.hasSetter(prop.getIndexedName())) {
        // 创建 MetaObject 对象
        MetaObject metaValue = metaObject.metaObjectForProperty(prop.getIndexedName());
        if (metaValue == SystemMetaObject.NULL_META_OBJECT) {// 如果 metaValue 为空，则基于 metaClass 判断是否有该属性的 getting 方法
          return metaClass.hasSetter(name);
        } else { // 如果 metaValue 非空，则基于 metaValue 判断是否有 getting 方法。
