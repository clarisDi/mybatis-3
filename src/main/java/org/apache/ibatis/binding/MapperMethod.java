...
public class MapperMethod {
  ...
  /**
   * SqlCommand 对象，是 MapperMethod 的内部静态类，SQL 命令
   */
  private final SqlCommand command;
  /**
   * MethodSignature 对象，是 MapperMethod 的内部静态类，方法签名。
   */
  ...
  public static class SqlCommand {
    ...
    public SqlCommand(Configuration configuration, Class<?> mapperInterface, Method method) {
      ...
      } else {  //找到 MappedStatement
        name = ms.getId(); //${NAMESPACE_NAME}.${语句_ID}
    ...
    private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
      ...
      } else if (mapperInterface.equals(declaringClass)) { // 如果没有，并且当前方法就是 declaringClass 声明的，则说明真的找不到
        return null;
      }
